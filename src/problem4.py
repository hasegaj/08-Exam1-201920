...
def problem4(number_of_stairs, step_size, starting_point, window):
    ...
    for k in range(number_of_stairs):
        starting_point = rg.Point(px, py)
        endpoint = rg.Point(px, py - step_size)

        line = rg.Line(starting_point, endpoint)
        line.color = 'magenta'
        line.thickness = 3
        line.attach_to(window)

        newspt = rg.Point(px + step_size, py - step_size)
        line2 = rg.Line(endpoint, newspt)
        line2.thickness = 3
        line2.attach_to(window)
        px = px + step_size
        py = py - step_size
    final_point = rg.Point(px, py)
    final_point.attach_to(window)
    window.render()
